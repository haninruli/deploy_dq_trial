...
df['order_year'] = df['order_date'].dt.year #extract year dr kolom order date
...
with mx_sales:
    curr_sales = data.loc[data['order_year']==CURR_YEAR, 'sales'].values[0] #tulisan values, supaya beneran dpt values, bukan list
...
with mx_order:
    curr_order = data.loc[data['order_year']==CURR_YEAR, 'order_id'].values[0] #tulisan values, supaya beneran dpt values, bukan list
...
with mx_customer:
    curr_customer = data.loc[data['order_year']==CURR_YEAR, 'customer_id'].values[0] #tulisan values, supaya beneran dpt values, bukan list
    prev_customer = data.loc[data['order_year']==PREV_YEAR, 'customer_id'].values[0]
    
    customer_diff_pct = 100.0 * (curr_customer - prev_customer) / prev_order
    
    st.metric("Customer", value=curr_customer, delta=f'{customer_diff_pct:.2f}%')

with mx_profit_pct:
    curr_profit = data.loc[data['order_year']==CURR_YEAR, 'profit_pct'].values[0] #tulisan values, supaya beneran dpt values, bukan list
    prev_profit = data.loc[data['order_year']==PREV_YEAR, 'profit_pct'].values[0]
    
    profit_diff_pct = curr_profit - prev_profit
    
    st.metric("Profit", value=f'{curr_profit:.2f}%', delta=f'{customer_diff_pct:.2f}%')
